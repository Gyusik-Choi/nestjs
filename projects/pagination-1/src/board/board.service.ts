...
export class BoardService {
  ...
  async search(queryParam: BoardSearchRequestDTO): Promise<Post<BoardSearchArticleDTO>> {
    const boards: [Board[], number] = await this.boardRepository.paging(queryParam);
    return new Post<BoardSearchArticleDTO>(boards[1], queryParam.getLimit(), boards[0].map(v => new BoardSearchArticleDTO(v)));
  }
}
